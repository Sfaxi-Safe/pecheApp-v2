...
class FishRecognitionService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      print('Modèle de reconnaissance de poissons initialisé avec succès');
    } catch (e) {
      print('Erreur lors de l\'initialisation du modèle de reconnaissance de poissons: $e');
      ...
      try {
        ...
        _isInitialized = true;
        print('Modèle initialisé avec CPU seulement');
      } catch (e) {
        print('Échec de l\'initialisation du modèle même avec CPU: $e');
        rethrow;
      }
    }
  }
  ...
  Future<Espece?> recognizeFish(File imageFile) async {
    ...
    try {
      ...
      if (maxProb < 0.5) {
        print('Confiance trop faible: $maxProb pour ${_labels[maxIndex]}');
        return null;
      }
      ...
      print('Erreur lors de la reconnaissance du poisson: $e');
  ...
  Future<List<List<double>>> _preprocessImage(File imageFile) async {
    ...
    for (int y = 0; y < inputSize; y++) {
      for (int x = 0; x < inputSize; x++) {
        ...
        // Extraire les composantes RGB et normaliser
        final r = (pixel & 0xFF) / 255.0;
        final g = ((pixel >> 8) & 0xFF) / 255.0;
        final b = ((pixel >> 16) & 0xFF) / 255.0;
