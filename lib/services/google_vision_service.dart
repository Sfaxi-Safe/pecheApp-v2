...
class GoogleVisionService {
  ...
  // Liste des espèces de poissons connues pour le mapping
  final Map<String, String> _fishSpeciesMapping = {
    ...
    'anchois': 'Anchois',
    'fish': 'Poisson non identifié',
  };
  
  GoogleVisionService._internal();
  
  /// Analyse une image pour identifier l'espèce de poisson
  Future<Espece?> identifyFish(File imageFile) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Erreur API Vision: ${response.statusCode} - ${response.body}');
        throw Exception('Erreur lors de l\'appel à l\'API Vision: ${response.statusCode}');
      }
    } catch (e) {
      ErrorHandler.instance.logError(e, context: 'GoogleVisionService.identifyFish');
      rethrow;
    }
  }
  ...
  Future<Espece?> _processApiResponse(Map<String, dynamic> apiResponse) async {
    try {
      ...
      final localizedObjectAnnotations = firstResponse['localizedObjectAnnotations'] ?? [];
      ...
      for (final label in labelAnnotations) {
        final String description = label['description'].toString().toLowerCase();
      ...
      fishCandidates.sort((a, b) => (b['score'] as double).compareTo(a['score'] as double));
      ...
      debugPrint('Espèce identifiée: $mappedSpecies (confiance: ${(confidence * 100).toStringAsFixed(1)}%)');
      
      // Vérifier si l'espèce existe dans la base de données
      try {
        final especeData = await ApiService.instance.getEspeceByNom(mappedSpecies);
      ...
      try {
        final newEspeceData = await ApiService.instance.createEspece(mappedSpecies);
        if (newEspeceData.containsKey('data') && newEspeceData['data'] != null) {
      ...
      return Espece(
        id: 'temp',
        nom: mappedSpecies,
        description: 'Identifié par Google Vision API avec ${(confidence * 100).toStringAsFixed(1)}% de confiance',
      );
    } catch (e) {
      ErrorHandler.instance.logError(e, context: 'GoogleVisionService._processApiResponse');
      return null;
    }
  }
